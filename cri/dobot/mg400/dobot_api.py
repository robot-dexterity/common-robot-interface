...
class DobotApiMove(DobotApi):
  ...
  """
    ...
    *dynParams :Parameter Settings（Mode、Distance、Index、Status）
    ...
    def ServoJ(self, offset1, offset2, offset3, offset4, *dynParams):
        """
        Dynamic follow command based on joint space
        j1~j4:Point position values on each joint
        """
        string = "ServoJ({:g},{:g},{:g},{:g}".format(offset1, offset2, offset3, offset4)
        for params in dynParams:
            string = string + ", t=" + str(params)
        string = string + ")"
        # print(string)
        return self.sendRecvMsg(string)

    def ServoP(self, offsetX, offsetY, offsetZ, offsetR, *dynParams):
        """
        Dynamic following command based on Cartesian space
        x, y, z, R :Cartesian coordinate point value
        """
        string = "ServoP({:g},{:g},{:g},{:g}".format(offsetX, offsetY, offsetZ, offsetR)  # does not like -0.00000
        for params in dynParams:
            string = string + ", t=" + str(params)
        string = string + ")"
        # print(string)
